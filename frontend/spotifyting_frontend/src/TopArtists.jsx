  ...
  return (
    <section className="bg-base-100 rounded-xl border-base-300 border py-12">
        <div className="container px-4">
            <h2 className="text-3xl font-bold mb-6 text-center">Your Top Artists</h2> 
            {artists ? (
                <ul className="list flex flex-col gap-2">
                    {Object.entries(artists).sort((a, b) => b[1] - a[1] ).map(([artist, count], index) => (
                        <li key={index} className="list-row flex items-center justify-between bg-base-200">
                            ...
                            <div className="font-semibold text-base-content">{artist}</div>
                            </div>
